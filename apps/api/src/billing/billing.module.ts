  ...
  async timbraComprobante(sede: any, cfdiXml: string): Promise<{
    ...
    try {
      const { data } = await axios.post(
        ...
        {
          usuario: pacUser, contrasena: pacPass,
...
export class SatService {
  ...
  async validateRfc(rfc: string): Promise<boolean> {
    ...
    try {
      ...
      return data?.estatus === 'A';
    } catch (e) {
      this.logger.warn(`SAT LCO no disponible para RFC ${rfc}: ${e.message}`);
      return true;
    }
  }
}
...
export class QuickBooksService {
  ...
  async doSync(facturaId: string): Promise<void> {
    await this.prisma.factura.findUniqueOrThrow({
      where: { id: facturaId },
      include: { cargos: true, pagos: true, paciente: true, sede: true },
    });
    this.logger.log(`[QB] Sync para factura ${facturaId} (implementación pendiente)`);
  }
...
export class BillingController {
  ...
  async create(@Body() dto: CreateInvoiceDto, @CurrentUser() u: any, @SedeId() s: string, @ClientIp() ip: string) {
    return this.svc.create(dto as any, u.userId, s, ip);
  }
  ...
  async findAll(
    ...
    @Query('estado') estado?: string, @Query('page') page = 1, @Query('limit') limit = 30,
  ) {
  ...
  async addCharge(@Param('id') id: string, @Body() dto: AddChargeDto, @CurrentUser() u: any, @SedeId() s: string) {
    return this.svc.addCharge(id, dto as any, u.userId, s);
  }
  ...
  async registerPayment(
    @Param('id') id: string, @Body() dto: RegisterPaymentDto,
    @CurrentUser() u: any, @SedeId() s: string, @ClientIp() ip: string,
  ) {
    return this.svc.registerPayment(id, dto as any, u.userId, s, ip);
  }
  ...
  @HttpCode(HttpStatus.OK)
  async cancel(
    @Param('id') id: string, @Body() dto: CancelCfdiDto,
    @CurrentUser() u: any, @SedeId() s: string, @ClientIp() ip: string,
  ) {
  ...
  @Roles(Rol.SUPERADMIN, Rol.ADMIN_SEDE, Rol.CAJA)
  @HttpCode(HttpStatus.OK)
  async closeCashRegister(@Body() dto: { turno: string }, @CurrentUser() u: any, @SedeId() s: string) {
