...
export const patientsApi = {
  ...
  timeline:       (id: string, page?: number)  => api.get(`/patients/${id}/timeline`, { params: { page } }),
...
export const hceApi = {
  ...
  // Notas — primer arg es consultaId, segundo es data
  createNota:     (consultaIdOrData: any, d?: any) =>
    d ? api.post(`/hce/consultas/${consultaIdOrData}/notas`, d)
      : api.post('/hce/notas', consultaIdOrData),
  updateNota:     (id: string, d?: any)        =>
    d ? api.patch(`/hce/notas/${id}`, d) : api.patch(`/hce/notas/${id}`, {}),
  signNota:       (id?: string)                =>
    api.post(`/hce/notas/${id ?? ''}/sign`, {}),
  // Vitales — acepta objeto con consultaId incluido o (id, data)
  upsertVitals:   (idOrData: any, d?: any)     =>
    d ? api.post(`/hce/consultas/${idOrData}/vitales`, d)
      : api.post(`/hce/consultas/${idOrData?.consultaId}/vitales`, idOrData),
  // Diagnósticos — acepta objeto con consultaId incluido o (id, data)
  addDiagnosis:   (idOrData: any, d?: any)     =>
    d ? api.post(`/hce/consultas/${idOrData}/diagnosticos`, d)
      : api.post(`/hce/consultas/${idOrData?.consultaId}/diagnosticos`, idOrData),
...
export const labApi = {
  ...
  getCatalog:     (q?: string)                 => api.get('/lab/catalogo', { params: q ? { q } : undefined }),
...
export const reportsApi = {
  ...
  conadic:        (anio: number, trimestre: number)  => api.get('/reports/conadic', { params: { anio, trimestre } }),
